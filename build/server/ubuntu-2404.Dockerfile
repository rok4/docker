...
ARG ROK4TILEMATRIXSETS_VERSION=4.7
...
RUN curl -L -o librok4-dev.deb https://github.com/rok4/core-cpp/releases/download/${ROK4CORECPP_VERSION}/librok4-ceph-${ROK4CORECPP_VERSION}-ubuntu-24.04-amd64.deb && apt install -y ./librok4-dev.deb

ARG ROK4SERVER_VERSION
ENV ROK4SERVER_VERSION=$ROK4SERVER_VERSION
RUN curl -L -o rok4-server.deb https://github.com/rok4/server/releases/download/${ROK4SERVER_VERSION}/rok4-server-${ROK4SERVER_VERSION}-ubuntu-24.04-amd64.deb && apt install -y ./rok4-server.deb
